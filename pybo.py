...
def index3():
    ...
    # 백현동, BM별 분포 Pie Data
    curs.execute(
        'select BM구분, count(*) from sampledb where substr(주소, 5, 11) = "성남시 분당구 백현동" and BM구분 IS NOT NULL group by BM구분 ORDER BY BM구분')
    ...
    # 판교동, BM별 분포 Pie Data
    curs.execute(
        'select BM구분, count(*) from sampledb where substr(주소, 5, 11) = "성남시 분당구 판교동" and BM구분 IS NOT NULL group by BM구분 ORDER BY BM구분')
    chart_value = curs.fetchall()
    list_chart3 = []
    print(chart_value)
    ...
    return render_template("index3.html", g22=rows, list_chart4=list_chart4, list_chart3=list_chart3,
                           list_chart2=list_chart2)
...
def index4():
    ...
    return render_template("index4.html", add2=rows, g22=rows2, list_chart4=list_chart4, list_chart3=list_chart3,
                           list_chart2=list_chart2, list_chart1=list_chart1)
...
def languages():
    ...
    curs = conn.cursor()
    curs.execute(
        'SELECT * FROM sampledb where 1=1 and (?="" OR 세대수=?) and (?="선택하세요" OR 지역=?) and (?="선택하세요" OR 건물유형=?)',
